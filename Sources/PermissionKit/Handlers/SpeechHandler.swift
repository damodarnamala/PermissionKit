...
#if canImport(Speech) && !os(tvOS) && !os(watchOS)
import Speech
import Foundation

/// Handles speech recognition permission using SFSpeechRecognizer.
final class SpeechHandler: PermissionHandler, @unchecked Sendable {

    let permission: Permission = .speechRecognition
    ...
    private static func mapStatus(_ status: SFSpeechRecognizerAuthorizationStatus) -> PermissionStatus {
        switch status {
